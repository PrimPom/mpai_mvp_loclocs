     ...
     <div >
      ...
      <div className="grid md:grid-cols-3 gap-10">
        {tools.map((tool, index) => (
          <div
            key={tool.id}
            className={`group relative rounded-xl p-6 transition-all duration-300 border border-transparent hover:bg-sky-600 hover:text-white ${
              index === 0 ? 'bg-primary text-white' : 'bg-transparent text-black hover:text-white'
            }`}
          >
            <span className="absolute top-4 right-4 text-lg font-semibold opacity-40 group-hover:opacity-100">
              {tool.id}
            </span>
            ...
            <button className="flex items-center gap-2 text-sm font-medium group-hover:underline">
